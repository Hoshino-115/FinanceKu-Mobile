...
public class HistoryActivity extends AppCompatActivity {
    ...
    private void showOptionsDialog(HistoryModel history) {
        String[] options = {"Ubah Nama", "Hapus"};
        AlertDialog.Builder builder = new AlertDialog.Builder(this);
        builder.setTitle("Opsi History");
        builder.setItems(options, (dialog, which) -> {
            if (which == 0) {
                showEditNameDialog(history);
            } else {
    ...
    private void showEditNameDialog(HistoryModel history) {
        final EditText input = new EditText(this);
        input.setText(history.nama);
        input.setSelection(history.nama.length());

        new AlertDialog.Builder(this)
                .setTitle("Ubah Nama History")
                .setView(input)
                .setPositiveButton("Simpan", (dialog, which) -> {
                    String newName = input.getText().toString().trim();
                    if (!newName.isEmpty()) {
                        dbHelper.updateHistoryName(history.id, newName);
    ...
    private void showDeleteDialog(HistoryModel history) {
        AlertDialog.Builder builder = new AlertDialog.Builder(this);
        builder.setTitle("Hapus History");
        builder.setMessage("Apakah Anda yakin ingin menghapus history '" + history.nama + "'? Ini juga akan menghapus semua detail transaksi di dalamnya.");
        builder.setPositiveButton("Hapus", (dialog, which) -> {
            dbHelper.deleteHistory(history.id);
            Toast.makeText(HistoryActivity.this, "History berhasil dihapus", Toast.LENGTH_SHORT).show();
            refreshData();
        });
